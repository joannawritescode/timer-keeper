	...
	output += '<div class="col" id="' + obj.id + '">';
		output += '<div class="card h-100">';
			output += '<div class="card-body">';
				output += '<div class="row m-0 p-0" style="margin-bottom: -20px!important;">';
					...
					output += '<div class="row mt-0">';
						output += '<div class="col-9 text-start position-relative">';
							output += '<h2 id="' + obj.id + '-title" text="' + obj.name + '" class="fw-bold text-nowrap">' + obj.name + '</h2>';
				...
				output += '</div>';
				// Time
				output += '<div id="' + obj.id + '-view" class="col d-flex justify-content-around align-items-end flex-column bg-primary-subtle text-center time p-2 ms-2" seconds="0">';
					
					output += '<hrs class="d-block" label="hours">0</hrs>';
					output += '<min class="d-block" label="minutes">0</min>';
					output += '<sec class="d-block" label="seconds">0</sec>';
				output += '</div>';

				output += '</div>';
				output += '<div class="row buttons text-center">';
					output += '<div class="col text-start d-flex align-items-center">';
						// buttons
						output += '<button class="delete-card btn btn-dark btn-hover-danger" id="' + obj.id + '-del" data-bs-placement="right" data-bs-toggle="tooltip" data-bs-title="Delete this timer."><i class="bi bi-trash-fill"></i></button>';
					output += '</div>';
					output += '<div class="col text-end">';
						output += '<button id="' + obj.id + '-btn" class="btn btn-primary w-100 fw-bold text-uppercase">Start</button>';
					output += '</div>';
				output += '</div>';
			output += '</div>';
		output += '</div>';
...
function Timer(key){
	...
	this.update = function() {
		this.display.children('hrs').html(this.hrs);
		this.display.children('min').html(this.min);
		this.display.children('sec').html(this.sec);
	}
